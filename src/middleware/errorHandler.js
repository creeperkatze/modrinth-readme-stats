...
// Modrinth logo SVG path
const MODRINTH_LOGO = (color) => `
...
function generateErrorCard(message, detailText = "", isCurseforge = false)
{
    const bgColor = "transparent";
    const errorTextColor = "#f38ba8";
    const detailTextColor = "#a6adc8";
    const accentColor = isCurseforge ? "#F16436" : "#1bd96a";
    ...
    const logo = isCurseforge
        ? `<svg x="15" y="15" width="24" height="24" viewBox="0 0 32 32">${CURSEFORGE_LOGO(accentColor)}</svg>`
        : `<svg x="15" y="15" width="24" height="24" viewBox="0 0 512 514">${MODRINTH_LOGO(accentColor)}</svg>`;
...
export async function errorHandler(err, req, res, next)
{
    ...
    {
        const svg = generateErrorCard(message, detailText, isCurseforge);
    ...
    } else
    {
        ...
        if (req.isImageCrawler)
        {
            res.status(200).send(generateErrorCard(message, detailText, isCurseforge));
        }
        else
        {
            res.status(statusCode).send(generateErrorCard(message, detailText, isCurseforge));
        }
    }
}
